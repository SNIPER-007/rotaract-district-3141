...
interface FolderStackProps {
  events?: readonly HomeFolderStackEvent[];
  className?: string;
}
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      cards.forEach((card, index) => {
        ...
        if (index === 0) {
          const initialFields = [date, title, description, cta].filter(Boolean);
        ...
        if (image) {
          ...
          timeline.to(image, { autoAlpha: 1, scale: 1.03, duration: 0.36, ease: "power3.out" }, start);
        }
      });
